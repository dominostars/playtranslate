#!/usr/bin/env bash
# Follow-up to rebuild_packs_v5.sh: rebuild all 23 Wiktionary source packs with
# three builder fixes and OVERWRITE the v4 assets in place.
#
#   A. The lexeme-aggregate frequency cut sums only the AGGREGATE_TOP_FORMS most
#      frequent forms[] surfaces, not all of them. Summing all of them stopped
#      the cut from cutting for languages with both a `large_` wordfreq list and
#      many forms per lemma — fi kept 156,345 lexemes against ~60k Finnish words
#      at or above 1e-6.
#   B. is_redirect_sense also fires on a form-of TAG and on a prose form-of
#      gloss, and pass 2 reads the target back out of that gloss. Pages saying
#      "apocopic form of suyo" with no structured field survived as lemmas and,
#      now that det/article/postp are content POS, shadowed the real word: es
#      `su` ranked ahead of `suyo`.
#   C. fi only: a forms[] surface earns an alias row only if wordfreq has seen
#      it. Defensible for a 600k-word list, not for a small one.
#
# ja-v5 is untouched and NOT rebuilt. zh, target/OCR/engine packs untouched.
#
# Overwriting is allowed HERE and ONLY for the tags in CLOBBER_OK, because no
# released app catalog references them — the only catalog that does is on this
# unmerged branch, and this run rewrites it. Every -v3 tag, hi-v4, ja-v4, ja-v5
# and zh-v2 stay frozen: older apps pin those by sha. upload() refuses any tag
# outside the two allowlists.
#
#   nohup bash scripts/rebuild_packs_v5b.sh > local/packs-v5b/logs/run.log 2>&1 &
set -uo pipefail

ROOT="/d/translate_app"
REPO="dominostars/playtranslate-langpacks"
BRANCH="ja-v5-keinf"
BASE_SHA="7801d65c5f2dfec142f766e2fd31bf0b931cc084"

BUILD="local/packs-v5b"
STAGE="$BUILD/stage"
TREE="$BUILD/tree"
LOGS="$BUILD/logs"
...
CLOBBER_OK=(ar-v4 ca-v4 da-v4 de-v4 en-v4 fr-v4 hu-v4 id-v4 it-v4 ko-v4 nl-v4 no-v4
            pl-v4 pt-v4 ro-v4 ru-v4 sv-v4 th-v4 tr-v4 vi-v4 hi-v5)
# Tags this run may CREATE (never existed).
CREATE_OK=(es-v4 fi-v4)
...
EXPECT_DIRTY=" M scripts/build_latin_dict.py
 ...
 M scripts/wiktionary_filters.py
?? scripts/rebuild_packs_v5b.sh
?? scripts/test_redirect_detection.py"
...
step "$(now) PHASE 0  stage last night's v4 for the reference delta"
for lang in "${SOURCES[@]}"; do
  [ "$lang" = "es" ] && continue
  [ -s "$V4REF/$lang.zip" ] && { echo "  $lang: v4 ref already staged"; continue; }
  if [ "$lang" = "fi" ]; then
    cp local/packs-v5/stage/fi-held/fi.zip "$V4REF/fi.zip" && echo "  fi: v4 ref from the held build"
    continue
  fi
  v=$(srcver "$lang")
...
for t in test_redirect_detection test_alias_chains test_misc_filter; do
