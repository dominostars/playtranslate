#!/usr/bin/env bash
# v5d: fix the rescue ordering and the letterless junk-gate edge, make the stub
# audit a permanent verify_pack gate, rebuild all 23 and overwrite again.
#
#   1. Redirect resolution is now a FIXPOINT. A chain can end at a page that only
#      becomes a lemma when the rescue emits it, so resolving once against
#      kept_lemma_ids left both the source and the terminal as stubs — hosted
#      en-v4 carried 2,127 of those (house lights -> house light -> houselight).
#      Resolve, emit the pages nothing can still walk to, re-resolve, repeat.
#   2. The alias junk gate no longer reads "has no letters" as "foreign script".
#      en "+1" -> "plus one" was an alias in v3 and had become stub lemmas, and
#      "three-sixty" never reached "360". Scoped to the REDIRECT pass: the forms[]
#      pass must keep rejecting letterless rows, because that same predicate gates
#      the frequency aggregate and word_frequency("1","nl") is 1.17e-03.
#   3. verify_pack gains a permanent stub-lemma gate, so this class of defect
#      fails the build instead of being found by opening a shipped pack.
#
# ja-v5 untouched and not rebuilt; zh, target, OCR and engine packs untouched.
# Overwriting stays safe because nothing from v5/v5b/v5c has reached users.
#
#   nohup bash scripts/rebuild_packs_v5d.sh > local/packs-v5d/logs/run.log 2>&1 &
set -uo pipefail

ROOT="/d/translate_app"
REPO="dominostars/playtranslate-langpacks"
BRANCH="ja-v5-keinf"
BASE_SHA="60e9c8ce14f41e56ce773a0d86f6aa53e756ad12"

BUILD="local/packs-v5d"
STAGE="$BUILD/stage"
TREE="$BUILD/tree"
LOGS="$BUILD/logs"
...
EXPECT_DIRTY=" M scripts/build_latin_dict.py
 ...
 M scripts/test_redirect_rescue.py
 M scripts/verify_pack.py
?? scripts/rebuild_packs_v5d.sh
?? scripts/test_verify_pack_stub_gate.py"
...
step "$(now) PHASE 0  stage the CURRENTLY HOSTED v5c assets as the reference delta"
...
for t in test_verify_pack_stub_gate test_redirect_rescue test_redirect_detection test_alias_chains test_misc_filter; do
