 ...
 *  - LEFT context is collected like forward context (capped at
 *    maxWords − 1 words), so a phrase is reachable from a tap on ANY of
 *    its member words — "open the door" from "door", "a great deal" from
 *    "deal".
 *
...
class LatinPhraseWindowTest {
    ...
    @Test fun `collects left context so mid-phrase taps see the whole phrase`() {
        // Tapping "great" must still see the full "a great deal" window —
        // anchored at "great".
        assertEquals(listOf("a", "great", "deal") to 1, window("a great deal", 2))
    }

    @Test fun `last word of a phrase sees its left context`() {
        // The containment contract's flagship: tapping "deal" (or "door" in
        // "open the door") reaches the phrase that ENDS at the tapped word.
        assertEquals(listOf("a", "great", "deal") to 2, window("a great deal", 8))
    }

    @Test fun `multi-letter left context is collected too`() {
        assertEquals(
            listOf("of", "a", "great", "deal") to 2,
            window("of a great deal", 5),
        )
    }

    @Test fun `left context is capped at maxWords minus one`() {
        assertEquals(
            listOf("four", "five", "six") to 2,
            window("one two three four five six", 24, maxWords = 3),
        )
    }

    @Test fun `left context respects chain breaks`() {
        // The comma detaches "great" — only the adjacent "a" is kept.
    ...
    @Test fun `word isolated by punctuation yields a singleton window`() {
        // A singleton can't form a phrase — longestPhraseAt returns null.
        assertEquals(listOf("deal") to 0, window("great, deal, of", 7))
    }
