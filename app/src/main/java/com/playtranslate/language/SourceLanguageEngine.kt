...
interface SourceLanguageEngine {
    ...
    /**
     * The longest multi-word dictionary expression starting at the word that
     * contains [offset] in [text], returned as a lookup key for [lookup] —
     * null when no known expression starts there. Word-tap surfaces call this
     * so a tap on "great" in "a great deal" resolves the expression instead
     * of the word (Yomitan's longest-match-from-cursor behavior).
     *
